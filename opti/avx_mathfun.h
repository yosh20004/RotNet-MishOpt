...
inline v8sf log256_ps(v8sf x) {
  ...
  imm0 = avx2_mm256_srli_epi32(_mm256_castps_si256(x), 23);       // 提取ieee754浮点数的高9位(1位符号位+8位阶码位)
                                                                          // imm0 = {000000000...}{符号位}{阶码} (整数)

  /* keep only the fractional part */
  x = _mm256_and_ps(x, *(v8sf*)_ps256_inv_mant_mask);                // ~0x7f800000 = 10000000011111111111111111111111 这个掩码得到了尾数
                                                                          // 此时x = 符号位 + 00000000 + 尾数
  x = _mm256_or_ps(x, *(v8sf*)_ps256_0p5);                           // ieee754(0.5f) = 0 01111110 0000000000000000000...
                                                                          // 此时x = 符号位 01111110 尾数 = {原数符号位}原数尾数部分 * 2^-1

  // this is again another AVX2 instruction
  imm0 = avx2_mm256_sub_epi32(imm0, *(v8si*)_pi32_256_0x7f);         // imm0 - 127 = 原数的真实阶码
                                                                          // ieee754的真实阶数 = 阶码 - 127
  v8sf e = _mm256_cvtepi32_ps(imm0);                                    

  e = _mm256_add_ps(e, one);                                         // e = 原数的真实阶码 + 1
                                                                          // x * 2^e = 原数
                                                                          // log(原数) = log(x) + e
  ...
  */                                                                      // 这里part2的算法就是: 如果x<SQRTHF, 则log(x) = log(2x/2)=log(2x)-1 我们计算log(2x)-1以代替log(x)
  
  //v8sf mask = _mm256_cmplt_ps(x, *(v8sf*)_ps256_cephes_SQRTHF);
  v8sf mask = _mm256_cmp_ps(x, *(v8sf*)_ps256_cephes_SQRTHF, _CMP_LT_OS); // 比较是否x<SQRTHF, 是则把mask对应位设置为全1
  v8sf tmp = _mm256_and_ps(x, mask);                                 // 如果x<SQRTHF，tmp = x & mask = x; 否则tmp = 0
  x = _mm256_sub_ps(x, one);                                         // x = x - 1
  e = _mm256_sub_ps(e, _mm256_and_ps(one, mask));               // 如果x<SQRTHF, e = e - 1; 否则e=e 
  x = _mm256_add_ps(x, tmp);                                         // x = x + tmp 到此完成了上述part2注释逻辑

  v8sf z = _mm256_mul_ps(x,x);

  // 接下来使用换底公式 + 泰勒展开计算log(x+1) 由于此时x已经经过了处理，log(x+1)+e就是原数的log
  // 换底公式: log(x+1) = ln(x+1) / ln2
  // 泰勒展开: ln(x+1) = x - x^2/2 + x^3/3 - x^4/4 + ...
  // 应用霍纳法则，将泰勒多项式展开为 (((((((((p0*x+p1)*x+p2)*x+p3)+p4)*x+p5)*x)+p6)*x+p7)*x+p8)*x
  // 实际上exp256那里也有类似式子，可以参照那里作为具体的计算过程 这样展开有利于快速计算多项式
...
_PS256_CONST(exp_hi,	88.3762626647949f);        // 令exp(x)不越界的最大x
_PS256_CONST(exp_lo,	-88.3762626647949f);

_PS256_CONST(cephes_LOG2EF, 1.44269504088896341);// 1/ln(2)
...
inline v8sf exp256_ps(v8sf x) {
  ...
  /* g = x-n*ln2  n = round(x/ln2) */
  fx = _mm256_mul_ps(x, *(v8sf*)_ps256_cephes_LOG2EF);      // fx = x / ln(2)
  fx = _mm256_add_ps(fx, *(v8sf*)_ps256_0p5);               // fx = x/ln2 + 0.5 (为了四舍五入做准备)

  /* how to perform a floorf with SSE: just below */
  //imm0 = _mm256_cvttps_epi32(fx);
  //tmp  = _mm256_cvtepi32_ps(imm0);
  
  tmp = _mm256_floor_ps(fx);                                     // tmp = [fx] (舍为较小整数，具体请查Intel文档) 
  ...
  fx = tmp;                                                      // fx=round(tmp) 四舍五入完成，这里我们还给原作者提了issue
                                                                 // fx = tmp = round(x/ln2) = n (整数)

  tmp = _mm256_mul_ps(fx, *(v8sf*)_ps256_cephes_exp_C1);    // tmp = fx*C1
  v8sf z = _mm256_mul_ps(fx, *(v8sf*)_ps256_cephes_exp_C2); // z = fx*C2
  x = _mm256_sub_ps(x, tmp);                                // x = x-fx*C1 (C1+C2=ln2 可能分解的原因是防止n*ln2将ln2的误差放大n倍)
  x = _mm256_sub_ps(x, z);                                  // x = x-fx*C1-fx*C2=x-fx/ln2 = x-n*ln2 = g

  z = _mm256_mul_ps(x,x);                                   // z = x*x = g^2
  
                                                                 // 计算e^g
  v8sf y = *(v8sf*)_ps256_cephes_exp_p0;                        
  y = _mm256_mul_ps(y, x);                                  // y = p0*g
  y = _mm256_add_ps(y, *(v8sf*)_ps256_cephes_exp_p1);      
  y = _mm256_mul_ps(y, x);                                  // y = (p0*g+p1)*g
  y = _mm256_add_ps(y, *(v8sf*)_ps256_cephes_exp_p2);
  y = _mm256_mul_ps(y, x);                                  // y = ((p0*g+p1)*g+p2)*g
  y = _mm256_add_ps(y, *(v8sf*)_ps256_cephes_exp_p3);
  y = _mm256_mul_ps(y, x);                                  // y = (((p0*g+p1)*g+p2)*g+p3)*g
  y = _mm256_add_ps(y, *(v8sf*)_ps256_cephes_exp_p4);
  y = _mm256_mul_ps(y, x);                                  // y = ((((p0*g+p1)*g+p2)*g+p3)*g+p4)*g
  y = _mm256_add_ps(y, *(v8sf*)_ps256_cephes_exp_p5);
  y = _mm256_mul_ps(y, z);                                  // y = (((((p0*g+p1)*g+p2)*g+p3)*g+p4)*g+p5)*(g^2)
  y = _mm256_add_ps(y, x);                                  // y = (((((((((p0*g)+p1)*g)+p2)*g)+p3)*g)+p4)*g)+p5)*(g^2)+g
  y = _mm256_add_ps(y, one);                                // y = ((((((((((p0*g)+p1)*g)+p2)*g)+p3)*g)+p4)*g)+p5)*(g^2)+g+1

  /* build 2^n */
  imm0 = _mm256_cvttps_epi32(fx);                              // 把n转为整数格式 
  // another two AVX2 instructions
  imm0 = avx2_mm256_add_epi32(imm0, *(v8si*)_pi32_256_0x7f); // n+127计算2^n的浮点数阶码(可回看计组笔记)
  imm0 = avx2_mm256_slli_epi32(imm0, 23);                    // 阶码位于第1位-第8位，共8位，所以需要左移13位
  v8sf pow2n = _mm256_castsi256_ps(imm0);                      // 显然2^n其他位数上全为0，所以(n+127)<<23 即ieee754(2^n) 本句告诉编译器不需物理上转换，只需解释为浮点数即可
  y = _mm256_mul_ps(y, pow2n);                               // e^x = 2^n + e^g
  return y;
}
